...
namespace TouchSocketCoreConsole
{
    internal class 插件
    {
        IPluginManager pluginsManager;
        public 插件()
        {
            pluginsManager = new PluginManager(new Container())
            {
                Enable = true//必须启用
            };
        ...
        public void Run() {
            while (true)
            {
                ...
                pluginsManager.Raise("Say", new object(), new MyPluginEventArgs()
                {
    ...
    public interface ISayPlugin : IPlugin
    {
        ...
        Task Say2(object sender, MyPluginEventArgs e);
    }
    ...
    public class SayHelloPlugin : PluginBase, ISayPlugin
    {
        ...
        public async Task Say2(object sender, MyPluginEventArgs e)
        {
            Console.WriteLine($"Say2:{e.Words}");
            await e.InvokeNext();
        }
    }
    ...
    {
        protected override void Loaded(IPluginManager pluginsManager)
        {
